...
export default function MemberTaskDetailPage() {
  ...
  return (
    <main className="min-h-screen bg-[#f7f6f6] p-8">
      <div className="max-w-4xl mx-auto">
        <button
          onClick={() => router.back()}
          className="text-sm text-[#404943] mb-6 hover:text-[#2d6a4f]"
        >
          ← Back
        </button>

        {loading && (
          <p className="text-[#404943]">Loading task...</p>
        )}
            ...
            <div className="bg-white rounded-xl shadow-sm p-6 mb-6">
              <div className="flex justify-between items-start">
                ...
                <select
                  ...
                  className={`text-xs font-medium rounded-full px-3 py-1.5 border-0 outline-none cursor-pointer ${
                  ...
                  >
                    <div className="flex justify-between text-xs text-[#404943] mb-1">
                      <span className="font-medium text-[#181d19]">
                        {c.userId?.name}
                        {c.userId?._id === currentUser?._id && " (you)"}
                      </span>
                    ...
                    <p className="text-sm text-[#181d19]">
                      {c.text}
                    </p>
                      ...
                      <div className="mt-1 space-y-1">
                        {c.attachments.map((url, i) => (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="block text-xs text-[#2d6a4f] underline break-all"
                          >
                            📎 {url}
                          </a>
                        ))}
